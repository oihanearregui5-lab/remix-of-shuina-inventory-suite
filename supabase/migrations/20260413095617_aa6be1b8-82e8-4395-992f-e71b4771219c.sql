...
-- Time entries table (the core of fichajes)
...
CREATE POLICY "Admins can view all entries" ON public.time_entries FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
