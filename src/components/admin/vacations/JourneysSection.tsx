  ...
  return (
    <>
      <div className="relative overflow-hidden rounded-xl border border-border bg-card">
        ...
        <div className="flex flex-wrap items-center gap-1.5 border-b border-border bg-muted/30 px-3 py-2 md:gap-2 md:px-5 md:py-4">
          <span className="mr-1 hidden text-[11px] font-bold uppercase tracking-[0.16em] text-muted-foreground md:inline">Trabajadores</span>
          <button
            type="button"
            onClick={() => setSelectedWorkerId(null)}
            className={cn(
              "flex-none rounded-full text-xs font-semibold transition-colors",
              "h-7 px-2.5 md:h-9 md:px-3 md:text-sm",
              !selectedWorkerId
          ...
          {allWorkers.map((worker) => {
            ...
            return (
              <button
                ...
                title={worker.name}
                className={cn(
                  "inline-flex flex-none items-center gap-1.5 rounded-full transition-all",
                  "h-7 px-1 md:h-9 md:px-2.5 md:text-xs",
                  isSelected ? "ring-2 ring-primary ring-offset-1 ring-offset-background" : "opacity-70 hover:opacity-100",
                )}
                style={{ backgroundColor: `${worker.color}26` }}
              >
                <span
                  className="flex h-6 w-6 flex-none items-center justify-center rounded-full text-[10px] font-extrabold leading-none md:h-6 md:w-6"
                  style={{ backgroundColor: worker.color, color: getContrastTextColor(worker.color) }}
                >
                  {initials.slice(0, 2)}
                </span>
                <span className="hidden pr-1.5 text-xs font-semibold text-foreground md:inline">
                  {worker.name?.split(" ")[0]}
                </span>
              </button>
            );
